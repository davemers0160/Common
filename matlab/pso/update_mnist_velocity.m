function V = update_mnist_velocity(X, V, P, G, c1, c2, kap, N, itr, pso_velocity_limits)
...
%   - v(k+1) = kap*(v(k) + c1*r(k) o (P(k) - x(k)) + c2*s(k) o (G - x(k)))
    ...
    for idx=1:N
        ...
        R = rand(size(X(idx,itr).fc));
        S = rand(size(X(idx,itr).fc));
        V(idx, itr+1).fc = kap * (V(idx,itr).fc + (c1*R).*(P(idx,itr).fc - X(idx,itr).fc) + (c2*S).*(G(itr).fc - X(idx,itr).fc));
...
%         V(idx, itr+1).crop_size = kap * (V(idx,itr).crop_size + (c1*R).*(P(idx,itr).crop_size - X(idx,itr).crop_size) + (c2*S).*(G(itr).crop_size - X(idx,itr).crop_size));
        ...
        V(idx, itr+1) = validate_mnist_pso_velocity(V(idx, itr+1), pso_velocity_limits);
    end
