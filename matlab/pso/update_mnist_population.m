function X = update_mnist_population(X, V, N, itr, pso_member_limits)
...
%   - x(k+1) = x(k) + v(k+1)
    ...
    for idx=1:N
        ...
        X(idx, itr+1).fc = X(idx,itr).fc + V(idx,itr+1).fc;
...
%         X(idx, itr+1).crop_size = X(idx,itr).crop_size + V(idx,itr).crop_size;
        ...
        X(idx, itr+1).fc_map = X(idx, itr).fc_map;
        
        X(idx, itr+1) = validate_mnist_pso_member(X(idx, itr+1), pso_member_limits);
    end
